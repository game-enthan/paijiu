...
-define(ROOM_TYPE_TIANJIU,109).
%% 玩家状态
-define(PLAYER_STATE_WAITING, 0).   %% 等待状态
...
-record(room_tianjiu,{
  ...
  has_guizi=false                   %% 鬼子
  hase_tianjiuwang=false            %% 天九王
  has_dijiuwang=false               %% 地九王
  has_sanbazha=false                %% 三八为炸弹
  chose_socre=true                  %% 分数选择(true=每次选分,false=固定分)
}).
...
-record(room_tuiduizi_property,{
  ...
  , is_river = false        %%是否压河
  , nine_double = false     %%9点翻倍
  , xian_double = false     %%闲翻倍
  , zhuang_double = false   %%庄翻倍
}).
