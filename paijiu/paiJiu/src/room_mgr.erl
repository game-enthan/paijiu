...
%% 创建房间 gps玩家位置
create_room(Type, OwnerId, RPid, Name, Icon, Sex, Ip, Gps, CostCardNum, IsAgentRoom, PayWay, BalanceTotalScore, BalanceTotalScore2, Property, Role) ->
    Mod = get_mod(Type),
    SeatList = get_seat_list(Type),
    {Seat, SeatList2} = 
    case IsAgentRoom of     %代理房间,返回座位号0和座位列表
        true -> 
            {0, SeatList};
        _ -> 
    ...
    %%     #player_tuiduizi{id = RoleId, pid = RPid, name = Name, seat_id = Seat, icon = Icon, sex = Sex, ip = Ip, gps = Gps}.
    Owner = Mod:create_player(OwnerId, RPid, Name, Seat, Icon, Sex, Ip, Gps, BalanceTotalScore, BalanceTotalScore2, Property),
    RoomId = get_room_id(0),    %得到一个六位数的房间号
    case Mod:start(RoomId, Type, Owner, CostCardNum, PayWay, Property) of
        {ok, Pid} -> %% Pid 是房间的fsm进程pid
            ...
            case IsAgentRoom of  %代理开的房是一直在的,打完一局房间仍然存在,其他人也可以加入这个房间,房卡扣的是开房间的人的
                false ->
...
get_room_id(Times) ->
    ...
    case ets:member(?ETS_ROOM, Id) of   %从ets(在内存中,速度快)查询是否有这个房间号,如果有则重新生成一个房间号,否则返回这个房间号
        true ->
            get_room_id(Times + 1);
        _ ->
            Id
